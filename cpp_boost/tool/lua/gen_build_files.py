...
#LUA_LIB
    ##replace MACRO ##
    replace_data=(
        ("#define LUA_CORE","#ifndef LUA_CORE\n#define LUA_CORE\n#endif/**/\n"),
        ("#define LUA_LIB","#ifndef LUA_LIB\n#define LUA_LIB\n#endif/**/\n"),
    );
...
def gen_build_files(rootPath):
    ...
    fileNames=gen_filenames(rootPath)

    if len(fileNames)<1:
        return

    out_dir_path=os.path.join(rootPath,"outs/lua")
    if os.path.exists(out_dir_path):
        print(out_dir_path)
        shutil.rmtree(os.path.join(rootPath,"outs"))
    ...
    for fileName in fileNames:
        if fileName[2]:#if this is a cpp file
            ...
            out_file_name=os.path.join(out_dir_path,fileName[1])#add boost_ for cpp files
